function [costo] = simplex_maximizar(f,A,b)
...
if (factibilidad_maximizar(cuadro) == 1)
    if (optimalidad_maximizar(cuadro,m) == 1)
        msgbox('cuadro óptimo')
    else
        while (optimalidad_maximizar(cuadro,m) ~= 1)
            [V1, p1] = min(cuadro(end,1+m+2:end-1));                    % Máximo valor columna pivote. V valor, P posición
